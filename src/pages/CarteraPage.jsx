...
const CarteraPage = () => {
  ...
  useEffect(() => {
    // Only search if searchTerm is not exactly equal to a selected ticker to avoid double opening
    if (searchTerm === newPosition.ticker && searchTerm !== '') return;
    ...
    const t = setTimeout(async () => {
      if (searchTerm.length > 1) {
        ...
        try {
          ...
          if (k) {
            const results = await eodhdService.searchTickers(searchTerm, k);
  ...
  }, [searchTerm, user, newPosition.ticker]);
  ...
  const handleAddPosition = async () => {
    if (!newPosition.ticker) {
      toast({ variant: "destructive", title: "Missing Ticker", description: "Please select an asset first." });
      return;
    }
    ...
    try {
      ...
      if (!asset) {
        const { data: a, error: assetError } = await supabase.from('assets').insert({
          ticker: newPosition.ticker,
          company_name: newPosition.name,
          asset_class: newPosition.type,
          currency: newPosition.currency,
          current_price: newPosition.entry_price
        }).select().single();
      ...
      let { data: portfolios } = await supabase.from('portfolios').select('id').eq('user_id', user.id).limit(1);
      let portfolioId;

      if (!portfolios || portfolios.length === 0) {
        const { data: newPortfolio, error: pError } = await supabase.from('portfolios').insert({
          user_id: user.id,
          name: 'Principal'
        }).select().single();
      ...
      const { error: posError } = await supabase.from('positions').insert({
        ...
        target_percentage: newPosition.target_percentage,
        custom_classification: newPosition.classification
      });

      if (posError) throw posError;

      toast({ title: "Position Added", description: `${newPosition.ticker} was added successfully.` });
          ...
          {brokers.map((broker) => (
            <div key={broker.id} className="relative group">
              <button
                ...
                className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all pr-8 ${selectedBrokerId === broker.id
